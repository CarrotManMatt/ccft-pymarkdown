...
It was created as a temporary wrapper to suppress false-positive failures from {labelled-url-pymarkdown} that have now been resolved.
Please install and make use of the upstream project instead.

image:https://img.shields.io/badge/%F0%9F%A5%95-CCFT--PyMarkdown-blue[CCFT-PyMarkdown,link={url-project-repository}]
image:https://img.shields.io/pypi/status/CCFT-PyMarkdown[PyPI Status,link={url-project-pypi}]
